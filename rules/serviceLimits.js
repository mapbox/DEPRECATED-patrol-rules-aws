...
module.exports.fn = function(event, context, callback) {
  ...
  function refreshCheck(callback) {
    support.refreshTrustedAdvisorCheck(params, function(err, data) {
      ...
      if (data) {
        ...
        case 'none':
        case 'abandoned':
          console.log('Refresh status none|abandoned, pausing for 15sec');
          setTimeout(refreshCheck(callback), 15000);
          break;
        case 'enqueued':
        case 'processing':
          console.log('Refresh status state is processing, pausing for 15sec');
          setTimeout(refreshCheck(callback), 15000);
          break;
        case 'success':
          getCheckResult(callback);
          break;
        default:
          return callback('ERROR: refresh check returned unknown status code');
          break;
        }
      }
    });
  ...
  function getCheckResult(callback) {
